...
app.post('/geocoder', async (req, res) => {
  ...
  const apiUrl = `https://api.route4me.com/api/geocoder.php?api_key=${apiKey}&addresses=${encodeURIComponent(address)}&format=${format}`;
  ...
  try {
    ...
    res.status(500).json({ message: 'Erro ao comunicar com a API do Route4Me' });
  }
});

app.post('/optimization_problem', async (req, res) => {
  const { route_name, optimize, addresses } = req.body;

  if (!Array.isArray(addresses) || addresses.length === 0) {
    return res.status(400).json({ message: 'Deve haver pelo menos um endereço' });
  }

  const route4MeDataRoute = { route_name, optimize, addresses };
  ...
  try {
    ...
    console.error('Erro ao se comunicar com a API do Route4Me:', error);
    res.status(500).json({ message: 'Erro ao se comunicar com a API do Route4Me' });
  }
});
...
app.post('/order', async (req, res) => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      res.status(response.status).json({ message: 'Erro na resposta da API do Route4Me' });
    }
  } catch (error) {
    console.error('Erro ao se comunicar com a API do Route4Me:', error);

    if (error.response) {
      res.status(error.response.status).json({ message: error.response.data.message || 'Erro desconhecido da API' });
    } else if (error.request) {
      res.status(500).json({ message: 'Sem resposta da API do Route4Me' });
    } else {
      res.status(500).json({ message: `Erro ao fazer requisição: ${error.message}` });
    }
  }
});
